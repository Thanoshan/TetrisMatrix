...
//using interrupts for the buttons, so a volatile type is neccesary. 
...
int colors[7][3] = {{7, 7, 0}, {7, 4, 0}, {4, 7, 0}, {7, 0, 0}, {0, 7, 0}, {0, 0, 7}, {0, 4, 0}};
  ...
  for (int row=0; row<32; row++){
    for (int col=0; col <16; col++){
      if(gridToDraw[row] & (1<<(15-col))) { //Logical shifts. So hot right now. Logical Shifts.
        matrix.drawPixel(col, row, matrix.Color333(colors[activeShape][0],colors[activeShape][1],colors[activeShape][2]));
      }
    }
  } 
}
...
void drawActivePiece(){
  for (int thisPixel=0; thisPixel < 4; thisPixel++){
    ...
    //if(activeShape = 
    matrix.drawPixel(pixelX + xOffset, pixelY, matrix.Color333(colors[activeShape][0],colors[activeShape][1],colors[activeShape][2]));
  } 
}
